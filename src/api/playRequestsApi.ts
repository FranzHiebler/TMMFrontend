...
export async function getPlayRequests(user: User): Promise<PlayRequestDto[]> {
  const res = await fetch(`${API}/PlayRequests`, { headers: authHeaders(user) });
...
export async function getMyPlayRequests(user: User): Promise<PlayRequestDto[]> {
  const res = await fetch(`${API}/PlayRequests/mine`, { headers: authHeaders(user) });
...
export async function createPlayRequest(
  ...
  const res = await fetch(`${API}/PlayRequests`, {
    method: "POST",
...
export async function convertPlayRequest(
  ...
  const res = await fetch(`${API}/PlayRequests/${id}/convert`, {
    method: "POST",
    headers: authHeaders(user),
    body: JSON.stringify(request),
  });
...
export async function closePlayRequest(id: string, user: User): Promise<void> {
  const res = await fetch(`${API}/PlayRequests/${id}/close`, {
    method: "POST",
    headers: authHeaders(user),
  });
